...
public class DbHelper extends SQLiteOpenHelper{
    ...
    public String  getCurrentNotification(String currTime, String day) {
        String message = "";

        SQLiteDatabase db = this.getWritableDatabase();
        Week week;
        Cursor cursor = db.rawQuery("SELECT * FROM ( SELECT * FROM "+TIMETABLE+" ORDER BY " + WEEK_FROM_TIME + " ) WHERE "+ WEEK_FRAGMENT +" LIKE '"+day+"%' AND "+WEEK_FROM_TIME+" LIKE '%"+currTime+"%'",null);
        while (cursor.moveToNext()){
            week = new Week();
            week.setId(cursor.getInt(cursor.getColumnIndex(WEEK_ID)));
            week.setSubject(cursor.getString(cursor.getColumnIndex(WEEK_SUBJECT)));
            ...
            week.setToTime(cursor.getString(cursor.getColumnIndex(WEEK_TO_TIME)));
            week.setColor(cursor.getInt(cursor.getColumnIndex(WEEK_COLOR)));
            if (week.getFromTime().contentEquals(currTime)) {
                message = week.getSubject() + " " + week.getFromTime() + " - " + week.getToTime() + " " + week.getRoom();
                cursor.close();
                db.close();
                return message;
            }
